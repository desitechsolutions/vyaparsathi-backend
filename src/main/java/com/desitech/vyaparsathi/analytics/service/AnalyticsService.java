...
public class AnalyticsService {
    ...
    public List<ItemDemandPredictionDto> predictItemDemand(Long itemId) {
        ...
        Map<Long, Integer> currentMonth = getVolume(shopId, now.minusMonths(1), now);
        Map<Long, Integer> prevMonth    = getVolume(shopId, now.minusMonths(2), now.minusMonths(1));
    ...
    public List<TopItemDto> getTopRisingFallingItems() {
        ...
        Map<Long, Integer> lastMonthMap = getSalesVolumeByVariant(shopId, lastMonthStart, now);
        Map<Long, Integer> prevMonthMap = getSalesVolumeByVariant(shopId, prevMonthStart, lastMonthStart);
    ...
    public List<SeasonalTrendDto> getSeasonalTrends() {
        ...
        Long shopId = TenantContext.getCurrentShopId();
        // Build a year-spanning window for seasonal data: last 12 months
        LocalDateTime oneYearAgo = LocalDateTime.now().minusMonths(12);
        Map<Integer, Long> monthSales = saleRepository
                .findSalesForAnalyticsByShop(shopId, oneYearAgo, LocalDateTime.now()).stream()
                .collect(Collectors.groupingBy(
        ...
        for (int month = 1; month <= 12; month++) {
            ...
            String season = (month == 12 || month <= 2) ? "Winter"
